...
void AtrFetcher::establishContext() {
    ...
    if (err != SCARD_S_SUCCESS) {
		EstEID_log("SCardEstablishContext ERROR: %x", err);
...
void AtrFetcher::listReaders() {
    ...
    if( err != SCARD_S_SUCCESS || !size ) {
		EstEID_log("SCardListReaders || !size ERROR: %x", err);
    ...
    if( err != SCARD_S_SUCCESS) {
		EstEID_log("SCardListReaders ERROR: %x", err);
        throw PcscException("list_readers");
    }
	...
	for (std::vector<wchar_t>::const_iterator i = readers.begin(); i != readers.end(); ++i) {
        ...
        if( !readerName.empty() ) {
			...
			EstEID_log("found reader: %ws", reader->getName().c_str());
...
void AtrFetcher::populateAtrs() {
    for (int i = 0; i<readerList.size(); i++) {
        ...
        try {
			...
			EstEID_log("reader %s threw exception: %s", reader->getName().c_str(), e.getErrorMessage().c_str());
        }
    }
}
...
void CardReader::connect() {
    ...
    if( err != SCARD_S_SUCCESS) {
		EstEID_log("SCardConnect ERROR for %s: %x", name.c_str(), err);
...
void CardReader::populateAtr() {
    ...
    if( err != SCARD_S_SUCCESS) {
		EstEID_log("SCardStatus ERROR for %s: %x", this->name.c_str(), err);
        throw PcscException("status");
    }
    bAtr.resize(atrSize);
    atr = BinaryUtils::bin2hex(bAtr);
	EstEID_log("Set ATR = %s for reader %ws", atr.c_str(), this->name.c_str());
}
