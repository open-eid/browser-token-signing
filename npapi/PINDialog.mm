    ...
    for (int retriesLeft = selected.retry; retriesLeft > 0; ) {
        ...
        if (selected.pinpad) {
            ...
            future = std::async(std::launch::async, [&]() {
                try {
                    std::vector<unsigned char> signature = PKCS11CardManager(p11.path).sign(selected, hash, nullptr);
        ...
        if (selected.pinpad) {
            ...
            try {
                std::vector<unsigned char> signature = PKCS11CardManager(p11.path).sign(selected, hash, dialog->pinField.stringValue.UTF8String);
